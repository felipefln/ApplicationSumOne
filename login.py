...
users = 'tempfile.temp' # This just sets the variable creds to 'tempfile.temp'
...
def Signup(): # This is the signup definition, 
    ...
    roots.title('Criar Usuário') # This renames the title of said window to 'signup'
    ...
    nameL = Label(roots, text='Novo Usuário: ') # This just does the same as above, instead with the text new username.
    ...
    signupButton = Button(roots, text='Criar', command=FSSignup) # This creates the button with the text 'signup', when you click it, the command 'fssignup' will run. which is the def
    signupButton.grid(columnspan=3, sticky=W)
    roots.mainloop() # This just makes the window keep open, we will destroy it soon
 
def FSSignup():
    with open(users, 'w') as f: # Creates a document using the variable we made at the top.
...
def Login():
    ...
    nameL = Label(rootA, text='Usuário: ') # More labels
    ...
    loginB = Button(rootA, text='Logar no Sistema', command=CheckLogin) # This makes the login button, which will go to the CheckLogin def.
    loginB.grid(columnspan=2, sticky=W)
 
    rmuser = Button(rootA, text='Excluir Usuário', fg='red', command=DelUser) # This makes the deluser button. blah go to the deluser def.
...
def CheckLogin():
    ...
    if nameEL.get() == uname and pwordEL.get() == pword: # Checks to see if you entered the correct data.
        ...
        r.title(':D')
        r.geometry('300x100') # Makes the window a certain size
        rlbl = Label(r, text='\n[+] Usuário Logado') # "logged in" label
...
def DelUser():
    os.remove(users) # Removes the file
